...
Запрос, которым страница Яндекса подгружает отзывы (`fetchReviews`), требует подписи `s` - 
её считает обфусцированный JS прямо в браузере, без неё Яндекс отвечает `400`. Разбирать 
этот алгоритм отдельно - долго и ненадёжно: перекомпилят фронт - и ревёрс-инженирингом
придётся заниматься снова.

Поэтому вместо прямых запросов используется headless-браузер (Playwright): он открывает 
страницу организации как обычный пользователь, скроллит список отзывов и просто слушает 
сетевые ответы, которые в этот момент делает сама страница. Подпись считает и 
...
3. Первые ~50 отзывов уже лежат прямо в HTML страницы при загрузке (засчёт SSR) - забираем их сразу,
   без скролла: достаточно было через DevTools посмотреть, какие HTML-элемент хранят в себе
   SEO-информацию, и на них ссылаться.
4. Дальше сервис скроллит список вниз и каждый раз ждёт новый ответ `fetchReviews`, пока не
   соберёт нужное число отзывов или пока список не закончится.
5. Если сеть оборвалась раньше, чем дошли до конца списка - возвращаем ошибку, а не отдаём
   часть данных под видом полного результата (что не совсем оптимальное решение!).
...
/api/settings` синхронно дёргает scraper, получает готовые данные и сохраняет их в БД. Пока
идёт запрос, фронтенд просто показывает, сколько секунд прошло, без polling.
...
Предлагаемый фикс: использование воркеров Laravel.
...
результат на уровне БД ПОКА смысла нет: в будущем, необходимость появится.
